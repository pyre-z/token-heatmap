...
try:
    from cache import render_cached
    from heatmap import (
        build_auto_svg,
        build_day_svg,
        build_month_svg,
    ...
    from theme_loader import get_themes, start_watch, stop_watch, theme_generation
...
def _svg_response(svg: str) -> Response:
    # Deterministic ETag from SVG content for revalidation-friendly caching
    digest = hashlib.sha256(svg.encode()).hexdigest()[:16]
    etag = f'"{digest}"'
    return Response(svg, media_type="image/svg+xml", headers={"Cache-Control": "public, no-cache", "ETag": etag})
...
@app.get("/token/readyz")
def readyz() -> JSONResponse:
    """就绪检查：配置有效且所选数据源可用；不暴露连接细节。"""
    try:
        source = get_source()
    except ValueError:
        return JSONResponse(status_code=503, content={"status": "not-ready", "reason": "invalid-source"})
    if not source.is_configured():
        return JSONResponse(status_code=503, content={"status": "not-ready", "reason": "source-unconfigured"})
    return JSONResponse(status_code=200, content={"status": "ready"})


def _render_svg(key: str, render: Callable[[], str]) -> Response:
    """带 TTL 缓存的 SVG 渲染：命中直接返回，未命中渲染后写入缓存。DB 错误不缓存。"""
    return _svg_response(render_cached(key, render))
...
def token_svg(grain: str = "auto", year: str | None = Query(None), month: str | None = Query(None), day: str | None = Query(None), theme: str = "github", lang: str = "zh", darkmode: str = "auto", bg: str = "0", scale: str = "1") -> Response:
    ...
    source = get_source()
    src_key = source.name
    key = f"{src_key}|{grain}|{year}|{month}|{day}|{theme}|{lang}|{scale:g}|{darkmode}|{bg}|{theme_generation()}"
    try:
        if grain == "auto":
            today = shanghai_today()
...
def config_page() -> str:
    ...
    return '''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>Token 用量热力图</title><style>body{font:14px sans-serif;max-width:760px;margin:40px auto}label{margin:8px;display:inline-block}input,select,button,textarea{padding:6px}textarea{display:block;width:100%;height:64px}img{display:block;max-width:100%;margin:20px 0}</style><h1>Token 用量热力图</h1><label><input type="radio" name="grain" value="auto" checked>最近一年</label><label><input type="radio" name="grain" value="year">年</label><label><input type="radio" name="grain" value="month">月</label><label><input type="radio" name="grain" value="day">日</label><label>主题 <select id="theme">__THEME_OPTIONS__</select></label><label>深浅 <select id="darkmode"><option value="auto">自动</option><option value="0">白天</option><option value="1">夜晚</option></select></label><label>背景 <select id="bg"><option value="0">透明</option><option value="1">主题色</option></select></label><label>语言 <select id="lang"><option value="zh">中文</option><option value="en">English</option></select></label><br><label>年份 <input id="year" type="number" min="2000" max="2100"></label><label>月份 <input id="month" type="number" min="1" max="12"></label><label>日期 <input id="day" type="number" min="1" max="31"></label>缩放 <input id="scale" type="number" min="0.1" max="10" step="0.1" value="1">（0.1-10x）</label><label><input id="current" type="checkbox" checked>使用当前日期</label><img id="preview"><textarea id="embed" readonly></textarea><button id="copy">复制嵌入代码</button><script>const q=s=>document.querySelector(s),now=new Date();for(const k of ["year","month","day"])q("#"+k).value=k==="year"?now.getFullYear():k==="month"?now.getMonth()+1:now.getDate();function render(){let g=q(\'input[name=grain]:checked\').value,p=new URLSearchParams({theme:q("#theme").value,lang:q("#lang").value,grain:g,darkmode:q("#darkmode").value});let sc=parseFloat(q("#scale").value);if(sc>=0.1&&sc<=10&&sc!==1)p.set("scale",String(sc));if(q("#bg").value==="1")p.set("bg","1");if(g==="auto"){q("#year").parentElement.style.display="none";q("#month").parentElement.style.display="none";q("#day").parentElement.style.display="none"}else if(g==="year"){q("#year").parentElement.style.display="";q("#month").parentElement.style.display="none";q("#day").parentElement.style.display="none"}else if(g==="month"){q("#year").parentElement.style.display="";q("#month").parentElement.style.display="";q("#day").parentElement.style.display="none"}else{q("#year").parentElement.style.display="";q("#month").parentElement.style.display="";q("#day").parentElement.style.display=""}let y=parseInt(q("#year").value),m=parseInt(q("#month").value),d=parseInt(q("#day").value);if(g!=="auto"){p.set("year",y);if(g==="month"){p.delete("day")}else{p.set("month",m);p.set("day",d)}}let dm=q("#darkmode").value,embed=\'\';if(dm==="auto"){embed=\'<img src="\'+location.origin+\'/token/@?\'+p.toString()+\'" alt="Token Heatmap">\'}else{embed=\'<img src="\'+location.origin+\'/token/@?\'+p.toString()+\'&darkmode=\'+dm+\'" alt="Token Heatmap">\'}q("#embed").value=embed;q("#preview").src="/token/@?"+p.toString()+"&darkmode="+dm}function copy(){navigator.clipboard.writeText(q("#embed").value).then(()=>{alert("已复制！")})}document.querySelectorAll("input[name=grain]").forEach(el=>el.addEventListener("change",render));q("#theme").addEventListener("change",render);q("#darkmode").addEventListener("change",render);q("#bg").addEventListener("change",render);q("#lang").addEventListener("change",render);q("#scale").addEventListener("input",render);q("#year").addEventListener("input",render);q("#month").addEventListener("input",render);q("#day").addEventListener("input",render);q("#current").addEventListener("change",function(){if(this.checked){const n=new Date();q("#year").value=n.getFullYear();q("#month").value=n.getMonth()+1;q("#day").value=n.getDate()}render()});q("#copy").addEventListener("click",copy);render();const opts=document.createElement("option");opts.textContent=document.querySelector("#theme").value;document.querySelector("#theme").selectedIndex=0</script></html>'''.replace("__THEME_OPTIONS__", theme_options)
