...
def _validate(grain: str, year: int | None, month: int | None, day: int | None, theme: str) -> tuple[int, int | None, int | None]:
    if grain not in {"year", "month", "day"}: _invalid()
    # theme 只接受主题族名（github 等），旧 github-dark 已不保留
    if theme not in THEMES: _invalid()
...
@app.get("/token/@")
def token_svg(grain: str = "year", year: str | None = Query(None), month: str | None = Query(None), day: str | None = Query(None), theme: str = "github", lang: str = "zh", darkmode: str = "auto", scale: str = "1") -> Response:
    """根据 grain 粒度生成年度、月度或日度 SVG。scale 控制缩放（0.1-10）；darkmode=0/1/auto。"""
    if darkmode not in {"0", "1", "auto"}:
        _invalid()
    try:
    ...
    key = f"{grain}|{year}|{month}|{day}|{theme}|{lang}|{scale:g}|{darkmode}"
    try:
        if grain == "year":
            return _render_svg(key, lambda: build_svg(year, fetch_daily_tokens(year), theme, lang, scale, darkmode))
        if grain == "month":
            return _render_svg(key, lambda: build_month_svg(year, month, fetch_daily_tokens_in_range(*month_epoch_bounds(year, month)), theme, lang, scale, darkmode))
        return _render_svg(key, lambda: build_day_svg(year, month, day, fetch_hourly_tokens(year, month, day), theme, lang, scale, darkmode))
